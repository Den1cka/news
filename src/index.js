...
function loadArticle(sourceId) {
    import(/* webpackChunkName: "article.component" */ "./article.component.js").then(({ default: ArticleComponent }) => {
        const article = new ArticleComponent("data-container", "alert-container", "exception-container", apikey, sourceId);
        article.loadArticlesAsync();
    });
}

function loadSource() {
    const source = new SourceComponent("data-container", "alert-container", "exception-container", apikey);
    source.onClick = (item) => {
        loadArticle(item.id);
    };
    source.loadSources();
